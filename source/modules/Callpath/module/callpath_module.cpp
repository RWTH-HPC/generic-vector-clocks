...
std::list<StackInfo> PNMPIMOD_Callpath_GetCallpath_Fct() {
    ...
    /**
     * @todo Quick hack, we update the stack here,
     *       this adds some unnecessary call stack levels
     *       and is not nice. The reason for that is that 
     *       the call stack module does not has shmem wrappers yet.
     *       I tried to add them, but ran into issues.
     *       So this needs some future investigation.
     */
