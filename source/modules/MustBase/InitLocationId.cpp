...
GTI_ANALYSIS_RETURN InitLocationId::init (MustLocationId *pStorage, const char* callName, int callId)
{
    ...
    if (newStack) {
      ...
      for (size_t i = 3; i < st.size(); ++i) {
        ...
        std::string object_func = trace.object_function;
        ...
        if (object_func.compare("__libc_start_main") == 0 ||
            object_func.compare("_start") == 0)
            break;
        ...
        levelInfo.symName = trace.object_function;
        levelInfo.fileModule = trace.source.filename;
        levelInfo.lineOffset = std::to_string(trace.source.line);
        thisFullLocation.stack.push_back(levelInfo);

        if (object_func.compare("main") == 0 ||
            object_func.compare("MAIN__") == 0)
          break;
      }
...
void InitLocationId::createHandleNewLocationCall (MustLocationId id, const char* callName, LocationInfo &location)
{
