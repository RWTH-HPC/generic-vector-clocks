...
if [ -f ./gen_api_shmem_1_0_f.h ]
then
    src=./gen_api_shmem_1_0_f.h
else
...
grep -e ' MPI_.*(.*)' -e ' make_cuda' -e ' cuda' -e ' shmem' -e 'alloc' -e 'free' $src \
| grep -v COMPLE | sed >$tmp.tmp \
    -e '/typedef /d' \
    -e 's/( *void *)/()/' \
    -e 's/   */ /g' \
	-e 's/const /const_/g' \
...
# $2 is the call name
  ...
  printf "\t\t\t\t\t<operation-arguments>\n"
  printf "\t\t\t\t\t\t<operation-argument op-arg-order=\"0\"><call-arg-name>USE_CALLNAME</call-arg-name></operation-argument>\n"
  printf "\t\t\t\t\t\t<operation-argument op-arg-order=\"1\"><call-arg-name>USE_CALLID</call-arg-name></operation-argument>\n"
  printf "\t\t\t\t\t</operation-arguments>\n"    	
  printf "\t\t\t\t</operation>\n"
...
gawk -f $tmp.nawk $tmp.xml > initial_shmem_specification.xml
