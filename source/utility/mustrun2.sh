...
#        ls -l $inst_arg/modules/
        ...
        echo ${nodesize_arg} ${fanin_arg} ${firstfanin_arg} ${do_dl} ${do_reproduce} ${do_ddt} ${do_capture} ${do_hybrid} ${do_tsan} ${stacktrace_arg}
...
get_prebuild_hash()
{
	...
	ls ${SPEC_DIR} | grep '.*\.xml$' | LC_ALL=C sort | sed -e s@^@${SPEC_DIR}/@g | xargs cat | md5sum -
        ...
        echo ${do_dl} ${do_reproduce} ${do_ddt} ${do_capture} ${do_hybrid} ${do_tsan}
	  )
...
End-of-File
				...
				# Add CUDA Checks if we have CUDA
				if [ "@CUDA_FOUND@" == "TRUE" ]
				then
					cat <<End-of-File >> $printLayoutFile
					<analysis name="CudaBasicChecks" group="MUST_CudaBasicChecks"/>
					<analysis name="CudaMemoryTransferChecks" group="MUST_CudaMemoryTransferChecks"/>
End-of-File
				fi
				# Add OpenSHMEM Checks if we have shmem
				if [ $do_shmem -eq 1 ]
				then
					cat <<End-of-File >> $printLayoutFile
					<analysis name="openSHMEMMemoryAccessChecks" group="MUST_openSHMEM"/>
End-of-File
				fi
            ...
            then
                ...
                then
                  ...
                  then
                    ...
                    <analysis name="VectorClock" group="MUST_OneSidedChecks" />
...
# root level analyses
        $[$layoutNumLayers-1])
            ...
            then
                cat <<End-of-File >> $printLayoutFile
                    <analysis name="OpenShmemTextAlignedColls" group="MUST_OpenShmemTextuallyAligned"/>
End-of-File
            fi
        ;;
...
if [ $do_shmem -eq 1 ] ; then
  grep -v 'MUST_DistributedDeadlock' $printLayoutFile | grep -v 'MUST_Deadlock' > $printLayoutFile.tmp; mv $printLayoutFile.tmp $printLayoutFile
fi
...
#We use the alternative handling in distributed mode; AND if we do not reproduce (in which case we dont run 
#checks) and if we dont do SHMEM, since that doesnt comes with DL checks just yet
if [ $do_distributed -eq 1 -a $do_reproduce -eq 0 -a $do_shmem -eq 0 ]
then
  if [ $do_dl -eq 1 ]    
  then
...
then
    shmemSpec=${mustSpecDir}/shmem_specification.xml
else
    shmemSpec=''
fi
...
if ! $1 \
  ...
  ${mustSpecDir}/shmem_vectorclock_wrapper.xml \
  $optionalSpecs \
  ${tsanspecs} \
  ${omptspec} \
    ...
    do
        ...
        then
            ...
            then
                ...
                then
                    temp="${temp} $<TARGET_FILE_NAME:@PNMPIZE_TARGET@> --fortran $executable"
                else
                    temp="${temp} $<TARGET_FILE_NAME:@PNMPIZE_TARGET@> $executable"
                fi
...
export TSAN_OPTIONS="$TSAN_OPTIONS ignore_noninstrumented_modules=1 exitcode=0"
# log_path=$temp_arg/tsan_log"
...
while  IFS= read -r i ; do
    ...
    case $opt in
    verbose | quiet | timing | massif | memcheck | nocrash | clean | cleanshm | hybrid | tsan | pnmpi-linked | unused | info | distributed | dl | fillnodes | isp | scorep | ddt | reproduce | nodl | capture | unique | inbuilddir | noprebuilt | printmem )
        envvars="${i%%=*} $envvars"
